...
\begin{itemize}
    \item \textbf{Development of \texttt{gipsDA} Package:} We provide a fully functional R~\cite{rcore2025} package that integrates \texttt{gips} methodology with the standard discriminant analysis workflow. The package is designed to be API-compatible with the widely used \texttt{MASS}~\cite{venables2002} library, ensuring immediate accessibility for practitioners.
...
\frac{f_{\mathbbm{X} | \Gamma = c, \mathbbm{Y} = \mathbbm{y}}\left(\mathbbm{x}\right) \cdot \mathbbm{P}\left(\Gamma = c, \mathbbm{Y} = \mathbbm{y} \right)}{\mathbbm{P}\left(\mathbbm{Y} = \mathbbm{y}\right) \cdot f_{\mathbbm{X} | \mathbbm{Y} = \mathbbm{y}}(\mathbbm{x})} \propto f_{\mathbbm{X} | \Gamma = c, \mathbbm{Y} = \mathbbm{y}}\left(\mathbbm{x}\right),
\end{equation}
conditioning $\mathbbm{X}$ directly on precision matrices we get:
\begin{equation}
...
\begin{itemize}
  ...
  \item[$f_X(x)$] probability density function (pdf) of $X$ evaluated at $x$.
