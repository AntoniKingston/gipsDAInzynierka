...
\begin{itemize}
    \item \textbf{Section \ref{sec:motivation}: Motivation and Goal} identifies specific limitations of renowed classifiers in high-dimensional settings, particularly the issues of parameter instability and singularity, and defines the primary objective of this thesis.
    \item \textbf{Section \ref{sec:symmetry}: Potential causes of Symmetries} provides rationale behind proposed solution, explaining why permutation invariance is plausible and valuable assumption in many real-world datasets.
%    \item \textbf{Section \ref{sec:da_desc}: General Description of Discriminant Analysis} briefly establishes the theoretical baseline, outlining the mechanics of LDA and QDA that will be modified in our work.
    \item \textbf{Section \ref{sec:contribution}: Contribution} summarizes specific achievements of this project, including development of \texttt{gipsDA} package and implementation of novel estimators.
\end{itemize}

\section{Motivation and Goal}
\label{sec:motivation}

Well-established generative classifiers, such as Linear Discriminant Analysis (LDA) and Quadratic Discriminant Analysis (QDA), remain fundamental tools in statistical machine learning~\cite{elemstats} due to their interpretability and closed-form solutions.
However, the performance of these methods is critically dependent on accurate estimation of class-conditional covariance matrices, denoted as $\boldsymbol{\Sigma}$.
...
This implies that model's complexity grows quadratically with the dimension of the feature space ($\Theta(p^2)$).
...
\begin{enumerate}
    ...
    \item \textbf{Estimator Singularity:} In extreme regimes where the number of features exceeds the sample size ($p > n$), the sample covariance matrix $\hat{\boldsymbol{\Sigma}}$ becomes rank-deficient. Consequently, it is no longer positive definite and, as such, not invertible. Since the decision functions for both LDA and QDA require computation of the precision matrix $\hat{\boldsymbol{\Sigma}}^{-1}$, classical algorithms break down in these scenarios.
\end{enumerate}
...
By integrating the methodology of \texttt{gips}~\cite{JSSv112i07} (Gaussian model Invariant by Permutation Symmetry) package, we aim to construct robust classifiers that impose statistically justified symmetry constraints on the covariance matrix.
...
\begin{itemize}
    ...
    \item Remains mathematically valid and computationally stable in $p > n$ regime by ensuring the estimated matrices are positive definite through symmetry projection.
    \item Offers a user-friendly interface compatible with standard R ecosystem, bridging the gap between advanced theoretical statistics and practical data science applications.
\end{itemize}

\section{Potential causes of Symmetries}
\label{sec:symmetry}

At first glance, the assumption that distinct features in a dataset share a permutation symmetry—meaning they remain invariant under specific reorderings—may appear overly restrictive~\cite{andersson1998symmetry}. In classical multivariate analysis, features are often treated as distinct entities with unique variances and pairwise correlations. However, in the context of high-dimensional data, the assumption of symmetry is not only mathematically convenient but often empirically justified by the underlying nature of data itself

The core may be the concept of \textit{exchangeability}. In many datasets, features are not arbitrary measurements but rather collections of comparable units. Consider following domains where symmetry naturally arises:

\begin{itemize}
    \item \textbf{Genomics and Bioinformatics:} In gene expression data, thousands of genes are measured simultaneously~\cite{dudoit2002comparison}. Genes belonging to the same biological pathway or functional group often exhibit similar behavior. It is statistically plausible to assume that correlation between any pair of genes within such a functional cluster is roughly equivalent, regardless of their specific labels.

    \item \textbf{Medical Diagnostics and Imaging:} Consider an analysis of digitized images for cancer diagnosis, such as Breast Cancer Wisconsin dataset used in~\cite{breast}. Features are computed from the characteristics of cell nuclei present in the image (e.g., radius, texture, smoothness)~\cite{street1993nuclear}. While each feature measures a distinct geometric property, groups of features derived from similar morphological aspects often display strong, structured correlations. Assuming a symmetric covariance structure among these related biometrics allows the model to capture the general "shape" of malignant cells without overfitting to the noise inherent in individual sample measurements.
...
Furthermore, the approach proposed in this thesis does not require prior knowledge of these specific relationships. Unlike rigid regularization methods that force the covariance matrix towards a specific target (such as the identity matrix), \texttt{gips} methodology is designed to \textit{discover} the optimal symmetry structure from the data itself~\cite{JSSv112i07}.
...
\begin{itemize}
    \item \textbf{Development of \texttt{gipsDA} Package:} We provide a fully functional R package that integrates \texttt{gips} methodology with the standard discriminant analysis workflow. The package is designed to be API-compatible with the widely used \texttt{MASS} library, ensuring immediate accessibility for practitioners.
...
Firstly, we revisit classical statistical machine learning algorithms for classification, specifically focusing on Linear Discriminant Analysis (LDA) and Quadratic Discriminant Analysis (QDA).
These well-established methods serve as the foundation for predictive models developed in this work~\cite{elemstats}.

Secondly, we introduce the methodology underlying \texttt{gips} (Gaussian model Invariant by Permutation Symmetry) package.
...
Integration of these two distinct areas, standard discriminant analysis and permutation symmetry discovery, constitutes the core contribution of this thesis.

\section{Theoretical description of LDA and QDA}

Linear Discriminant Analysis (LDA) and Quadratic Discriminant Analysis (QDA) are generative classifiers that model the observations from each class, $g$, as a Gaussian distributed random variable, $X|G = g \sim \mathcal{N}(x | \mu_g, \Sigma_g)$. The key difference between them lies in their assumptions regarding the class covariance matrices, $\Sigma_g$.
...
Prediction for a new observation $x$ is made by assigning it to the class $g$ that maximizes posterior probability $\mathbbm{P}(g|x)$. Using Bayes' theorem, this is equivalent to maximizing the discriminant function, $\delta_g(x)$:
\begin{equation}
    \delta_g(x) = -\frac{1}{2} \log |\Sigma_g| - \frac{1}{2} (x - \mu_g)^T \Sigma_g^{-1} (x - \mu_g) + \log \pi_g,
\end{equation}
where $\mu_g$ is mean vector for class $g$, $\Sigma_g$ is its covariance matrix, and $\pi_g$ is the prior probability of the class. In practice, these parameters are estimated from the training data. For a more detailed treatment of these methods, we refer the reader to "The Elements of Statistical Learning"~\cite{elemstats}. Our work focuses on novel estimation techniques for the $\Sigma_g$ matrices.


\section{General description of gips}
Package \texttt{gips} provides a framework for identifying and
...
We modify the standard Linear Discriminant Analysis (LDA) and Quadratic Discriminant Analysis (QDA) algorithms by leveraging the methodology of \texttt{gips} R package~\cite{JSSv112i07}.
The core functionality of the library is to identify the most probable permutation group under which a given covariance matrix remains approximately invariant.
...
Let $K \in \mathbb{R}^{p\times p}$ and $\Gamma \leq \mathfrak{S}_p$. We say that matrix $K$ is invariant under action of subgroup $\Gamma$ iff
\begin{equation}
...
We attach full derivation of \eqref{gipsmultposteriorformula} in \autoref{app:gipsmult_derivation}.



\section{Description of machine learning models}
Building upon the theoretical framework of permutation-invariant covariance estimation established in the previous section, we now turn to the practical application of these concepts in supervised learning. The primary objective of this section is to define a new family of classifiers that integrate \texttt{gips} methodology into the classical frameworks of Linear and Quadratic Discriminant Analysis.
...
\subsection{gipsQDA}
The \texttt{gipsQDA} model represents the most flexible approach within our framework. In this method, \texttt{gips} library is applied independently to the data of each class. This process yields a unique estimated covariance matrix, $\hat{\Sigma}_g$, and a unique optimal permutation group, $\hat{c}_g$, for each class $g$:
\begin{equation}
...
\subsection{gipsLDA}
The \texttt{gipsLDA} model adapts the core idea of LDA—a single, shared covariance matrix—to \texttt{gips} framework. The general strategy is to first pool the covariance information from all classes into a single matrix, and then use \texttt{gips} to find a common symmetry structure for this pooled matrix. The process is as follows:
\begin{enumerate}
...
Model \texttt{gipsLDA} is contained within \texttt{gipsmultQDA}, while simultaneously being a specific case of \texttt{LDA}. All models are subsets of the general \texttt{QDA}.

\begin{figure}[ht]
\centering
\begin{tikzpicture}

\draw[fill=gray, fill opacity=0.05] (1,0) ellipse (9cm and 5.5cm);
\node [above=48mm] at (1,0) {\textbf{\texttt{QDA}}};

\draw[fill=blue, fill opacity=0.15] (-1,0) ellipse (5.5cm and 4cm);
\node at (-4, 2.5) {$\texttt{gipsQDA}$};

\draw[fill=orange, fill opacity=0.15] (4.3,0) ellipse (4.7cm and 4cm);
\node at (6.8, 2.5) {$\texttt{LDA}$};

\draw[fill=green, fill opacity=0.15] (0,0) ellipse (3.8cm and 2.8cm);
\node at (-1.5, 1.8) {$\texttt{gipsMultQDA}$};

\draw[fill=red, fill opacity=0.25] (1.5,0) ellipse (1.8cm and 1.2cm);
\node at (1.5,0) {$\texttt{gipsLDA}$};

\end{tikzpicture}
\caption{The diagram illustrates the hierarchical relationships between the models.}
...
A utility file (\texttt{models\_utils.R}) contains the core bridge functions that connect the modified \texttt{MASS} code to \texttt{gips} logic. The implementation was streamlined to use a single, versatile function that handles both estimation strategies.
...
\noindent where $\mathbb{I}(\cdot)$ is the indicator function, taking the value $1$ if the condition $y_i = \hat{y}_i$ is met (i.e., the prediction is correct) and $0$ otherwise. This metric provides a global measure of the model's effectiveness in correctly identifying class membership across the entire test set.
...
\noindent where $\text{AUC}(c_i, c_j)$ represents the probability that a randomly chosen member of class $c_i$ will have a higher estimated probability of belonging to class $c_i$ than a randomly chosen member of class $c_j$. This metric is robust to class imbalance and provides a coherent measure of separability across all class boundaries.
...
\begin{description}
    ...
    \end{itemize}
\end{description}
In every scenario scenario $n_k$ (number of observations generated from class k) ranges between $\left\lceil\frac{16}{n_{classes}}\right\rceil$ and 50.
...
\begin{enumerate}
    ...
    \item We sampled the majority class (Legitimate) such that the final dataset composition was 40\% Fraud and 60\% Legitimate. It should be noted that \texttt{MASS} LDA and QDA implementations estimate priors based on passed data, as such in this case $\pi_{FALSE} = 0.6$ and $\pi_{TRUE} = 0.4$.
...
Wilcoxon test is fundamentaly a one-sample test, as we are dealing with 2 samples we first create a single sample $\mathbbm{X}$ by subtracting the second one from the first:
\begin{equation}
...
\begin{itemize}
    \item \textbf{\texttt{gipsldawa}}: The \texttt{gipsLDA} model utilizing the \textbf{weighted average} estimator (Covariance Mtrices by Class Averaging).
...
\begin{itemize}
    ...
    \item \textbf{For $p=10$:} For $p=10$, while exhaustive enumeration of the permutation space ($10! \approx 3.6 \times 10^6$) is technically feasible for a single instance (taking approximately 20 minutes), it becomes computationally prohibitive for large-scale benchmarking requiring thousands of repetitions. Consequently, for dimensions $p \ge 10$, we use the stochastic \textbf{Metropolis-Hastings} algorithm.
\end{itemize}
...
\begin{table}[H]
    ...
    % --- SCENARIO: LDA ---
    \multirow{2}{*}{lda}
      & \multirow{1}{*}{LDAmod} & \multirow{1}{*}{gipsLDA classic} & 74 & 0.0656 \\
      \cmidrule{2-5}
      & \multirow{1}{*}{QDAmod} & \multirow{1}{*}{gipsMultQDA} & 74 & < 0.0001 \\
    \midrule
    % --- SCENARIO: GIPSLDA ---
    \multirow{3}{*}{gipslda}
      & \multirow{1}{*}{LDAmod} & \multirow{1}{*}{gipsLDA classic} & 74 & < 0.0001 \\
      \cmidrule{2-5}
      & \multirow{1}{*}{LDAmod} & \multirow{1}{*}{gipsLDA weighted} & 74 & 0.0017 \\
      \cmidrule{2-5}
      & QDAmod & gipsMultQDA & 74 & < 0.0001 \\
    \bottomrule
    \end{tabular}
...
\begin{table}[H]
    ...
    \multirow{3}{*}{qda}
      & \multirow{1}{*}{LDAmod} & \multirow{1}{*}{gipsLDA classic} & 74 & < 0.0001 \\
      \cmidrule{2-5}
      & \multirow{1}{*}{LDAmod} & \multirow{1}{*}{gipsLDA weighted} & 74 & < 0.0001 \\
      \cmidrule{2-5}
      & QDAmod & gipsMultQDA & 74 & < 0.0001 \\
    \midrule
    % --- SCENARIO: GIPSQDA ---
...
\textbf{Observations:}
In numerous scenarios, we observe the rejection of the null hypothesis, signifying the supremacy of \texttt{gips} models.
It is important to note that sample sizes of 16 and 34 were insufficient for the models to successfully fit.
While the performance advantage is backed by statistical testing on larger samples, the overall appearance of the accuracy curves and boxplots does not reveal a substantial visual difference.
...
\begin{table}[H]
    ...
    \multirow{9}{*}{qda}
      ...
      & & & 36 & < 0.0001 \\
      \cmidrule{2-5}
      & \multirow{3}{*}{LDAmod} & \multirow{3}{*}{gipsLDA weighted} & 16 & < 0.0001 \\
      & & & 24 & < 0.0001 \\
      & & & 36 & < 0.0001 \\
      \cmidrule{2-5}
      & \multirow{3}{*}{QDAmod} & \multirow{3}{*}{gipsMultQDA} & 16 & < 0.0001 \\
      & & & 24 & < 0.0001 \\
      & & & 36 & < 0.0001 \\
    \midrule
    % --- SCENARIO: GIPSQDA ---
...
\begin{table}[H]
    ...
    % --- SCENARIO: LDA ---
    \multirow{1}{*}{lda}
      & \multirow{1}{*}{QDAmod} & \multirow{1}{*}{gipsMultQDA} & 74 & 0.0071 \\
    \midrule
    % --- SCENARIO: GIPSLDA ---
    \multirow{3}{*}{gipslda}
      & \multirow{1}{*}{LDAmod} & \multirow{1}{*}{gipsLDA weighted} & 74 & 0.0013  \\
      \cmidrule{2-5}
      & \multirow{1}{*}{gipsLDA classic} & \multirow{1}{*}{gipsLDA weighted} & 74 & 0.0094 \\
      \cmidrule{2-5}
      & \multirow{1}{*}{QDAmod} & \multirow{1}{*}{gipsMultQDA} & 74 & 0.0001 \\
    \bottomrule
    \end{tabular}
...
\begin{table}[H]
    ...
    % --- SCENARIO: GIPSQDA ---
    \multirow{3}{*}{gipsqda}
      & \multirow{1}{*}{LDAmod} & \multirow{1}{*}{gipsLDA classic} & 74 & 0.0618 \\
      \cmidrule{2-5}
      & \multirow{1}{*}{LDAmod} & \multirow{1}{*}{gipsLDA weighted} & 74 & 0.0183 \\
      \cmidrule{2-5}
      & \multirow{1}{*}{QDAmod} & \multirow{1}{*}{gipsMultQDA} & 74 & 0.0004 \\
    \midrule
    % --- SCENARIO: GIPSMULTQDA ---
    \multirow{3}{*}{gipsmultqda}
      & \multirow{1}{*}{LDAmod} & \multirow{1}{*}{gipsLDA classic} & 74 & 0.0009\\
      \cmidrule{2-5}
      & \multirow{1}{*}{LDAmod} & \multirow{1}{*}{gipsLDA weighted} & 74 & 0.0696 \\
      \cmidrule{2-5}
      & \multirow{1}{*}{QDAmod} & \multirow{1}{*}{gipsMultQDA} & 74 & < 0.0001 \\
    \bottomrule
    \end{tabular}
...
\textbf{Observations:}
The significant disparity between dimensionality and the number of classes renders this scenario considerably more challenging than the previous two.
This complexity manifests as lower accuracy plateaus and distinct performance gaps between models. Critically, sample sizes of 16 and 34 observations proved insufficient for the models to successfully fit parameters.
Consequently, the statistical test results, consistent with the visual trends, are only applicable to larger sample sizes.
...
\begin{table}[H]
    ...
    \begin{tabular}{llc}
        ...
        LDAmod vs gipsLDA classic & 20 & $<0.0001$ \\
                                  ...
                                  & 76 & $<0.0001$ \\
        \midrule
        LDAmod vs gipsLDA weighted average & 20 & $<0.0001$ \\
                                           & 35 & $<0.0001$ \\
                                           & 76 & $0.4091$ \\
        \midrule
        ...
        QDAmod vs gipsQDA & 20 & $0.0157$ \\
                          ...
                          & 76 & $0.4234$ \\
        \midrule
        QDAmod vs gipsMultQDA & 20 & $<0.0001$ \\
                              & 35 & $<0.0001$ \\
                              & 76 & $0.0066$ \\
        \bottomrule
    \end{tabular}
...
\begin{itemize}
    ...
    \item \textbf{The "Classic" vs. "Weighted" Dilemma:} One of the most significant findings was the underperformance of weighted average strategy in the homoscedastic \texttt{gipsLDA} model. The models are theoreticaly asymptoticaly equivalent the issue may be an external factor like numerical instability.
...
\begin{itemize}
    ...
    \item \textbf{Advanced Optimization Strategies:} For dimensions $p \ge 10$, the current Metropolis–Hastings sampler utilizing transposition-based proposals exhibits slow mixing. Future work should investigate more efficient discrete optimization and sampling techniques to surpass the performance of the standard MH algorithm. Potential avenues include alternative proposal distributions, simulated annealing, or evolutionary algorithms.
\end{itemize}
% ======================================================================
%                          Rest
% ======================================================================
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{ >{\raggedright\arraybackslash}p{6cm} X X }
        \toprule
        \textbf{Role} & \textbf{Norbert Frydrysiak} \newline 327273 & \textbf{Antoni Kingston} \newline 327284 \\
        \midrule
        % --- WRITING DRAFT ---
        ...
        \addlinespace[10pt]
        \textbf{Software}  &
        Modules: \newline unit tests suite and \texttt{models} &
        Modules: \newline \texttt{gipsmult} and \texttt{models} \\
        \addlinespace[10pt]
        \textbf{Key solutions of the work} \newline
        \textit{Conceptualization, Investigation, Methodology} &
        Mathematical derivation of the \texttt{gipsmult} module; Designing the package API. \newline
        Creating unit tests; Proposing the methodology for experiments on synthetic and real-world data. &
        Mathematical derivation of the \texttt{gipsmult} module; Designing the package API. \newline
        Proposing the covariance matrix sampling procedure; Proposing covariance matrix regularization. \\
        \bottomrule
    \end{tabularx}
    \caption{The contribution of the authors of the thesis - Part 1}
\end{table}
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{ >{\raggedright\arraybackslash}p{6cm} X X }
        ...
        (including explanatory data analysis) and Validation &
        Execution and analysis of experiments. \newline
        Explanatory Data Analysis (EDA) for real-world datasets. \newline
        Code validation via unit tests. &
        Execution and analysis of experiments. \newline
        Conducting statistical tests. \newline
        Manual validation of \texttt{gipsmult} and \texttt{models} modules. \\
        \addlinespace[10pt]
        \textbf{Data curation} &
        YES &
        YES \\
        ...
        \bottomrule
    \end{tabularx}
    \caption{The contribution of the authors of the thesis - Part 2}
\end{table}
...
\begin{equation}
p\left( \Gamma = c \mid \mathbbm{X} = \mathbbm{x}, \mathbbm{Y}=\mathbbm{y}\right) =
\frac{p\left(\mathbbm{X} = \mathbbm{x} \mid \Gamma = c, \mathbbm{Y} = \mathbbm{y} \right) p\left(\Gamma = c, \mathbbm{Y} = \mathbbm{y} \right)}{p\left(\mathbbm{X} = \mathbbm{x}, \mathbbm{Y} = \mathbbm{y}\right)} \propto p\left(\mathbbm{X} = \mathbbm{x} \mid \Gamma = c, \mathbbm{Y} = \mathbbm{y} \right),
\end{equation}
phrasing this in more probabilistic terms and conditioning $\mathbbm{X}$ directly on precision matrices we get:
\begin{equation}
p\left( \Gamma = c \mid \mathbbm{X} = \mathbbm{x}, \mathbbm{Y}=\mathbbm{y}\right) \propto f_{\mathbbm{X} \; \mid \; \Gamma = c, \; \mathbbm{Y} = \mathbbm{y}}(\mathbbm{x}) = \idotsint\limits_{\mathcal{P}_c^m}
...
\begin{equation}\label{UltimateAPosterioriFormula}
    p\left(\Gamma = c \mid \mathbbm{X} = \mathbbm{x}, \mathbbm{Y} = \mathbbm{y}\right) \propto
...
\begin{itemize}
  \item[$\mathcal N(\mu,\Sigma)$] multivariate normal distribution.
  We write $X\sim \mathcal N(\mu,\Sigma)$ for a random vector with mean $\mu$ and covariance matrix $\Sigma$
  (optionally: in $\mathbb R^p$ we write $\mathcal N_p(\mu,\Sigma)$).

  \item[$\Sigma$] covariance matrix.

  \item[$K$] precision matrix, defined as the inverse covariance: $K=\Sigma^{-1}$ (when $\Sigma$ is invertible).

  \item[$|A|$] determinant of a square matrix $A$, i.e.\ $|A|=\det(A)$ (in particular $|\Sigma|=\det(\Sigma)$).

  \item[$\operatorname{tr}(\cdot)$] trace of a square matrix (sum of diagonal entries).

  \item[$I_p$] identity matrix of size $p\times p$ (we may write $I$ when the dimension is clear).

  \item[$\mathbf 1(\cdot)$] indicator function: $\mathbf 1(A)=1$ if the statement/event $A$ is true and
  $\mathbf 1(A)=0$ otherwise.

  \item[$\operatorname{sgn}(\cdot)$] sign (signum) function:
  \[
    \operatorname{sgn}(x)=
    \begin{cases}
      -1 & \text{if } x<0,\\
  ...
  \item[$\propto$] “proportional to”; $f(x)\propto g(x)$ means $f(x)=c\,g(x)$ for some constant $c>0$
  that does not depend on $x$.

  \item[$\mathfrak{S}_p$] symmetric group on $\{1,\ldots,p\}$, i.e.\ the set of all permutations of $\{1,\ldots,p\}$
  (with composition as the group operation).

  \item[$\Gamma \le \mathfrak{S}_p$] a subgroup $\Gamma$ of the symmetric group $\mathfrak{S}_p$.
\end{itemize}
\thispagestyle{empty}
