...
After installation one can import package in the standard R way and call it's functions. In this case gipsqda() is used to build a classifier on widely known \textit{Iris} dataset\cite{iris_53}.
...
The project utilizes the \textbf{\texttt{testthat}} framework ($\ge$ 3.3.1) along with \texttt{mockery} ($\ge$ 0.4.5) for mocking objects to ensure code reliability.
Documentation was written in the form of \texttt{roxygen2} comments and rendered using \texttt{pkgdown}. Github actions were configured to run tests and generate documentation automatically upon any changes to the main branch
...
We conducted an extensive simulation study comprising over 100 independent large-scale experiments. These experiments explored a vast grid of hyperparameters, including varying dimensions ($p$), numbers of classes ($k$), eigenvalue distributions (Exponential, Log-Normal, Chi-squared), and optimization strategies (MAP vs. BMA). From this extensive collection, we selected \textbf{representative configurations} (parameter sets) for detailed discussion in this chapter. These selections best illustrate the specific strengths and limitations of the proposed methods.
...
\textbf{Observations:}
In this binary classification setting ($p=10, k=2$) with Chi-squared distributed data, the benefits of permutation-based regularization are profound.
...
\textbf{Observations:}
In this low-dimensional, multi-class setting ($p=5, k=10$) with Log-Normal distributed data, the distinction between models is more nuanced than in higher-dimensional scenarios.
...
\textbf{Analysis:}
Despite the application of One-Hot Encoding, which introduces binary features and explicitly violates the multivariate normality assumption, the \texttt{gips}-based models demonstrate remarkable robustness and data efficiency. The statistical analysis reveals that \texttt{gipsLDA classic} significantly outperforms the regularized \texttt{LDAmod} baseline ($pvalue < 0.001$), confirming that the discovered symmetry structure aids in stabilization even for non-Gaussian data. Furthermore, within the quadratic family, \texttt{gipsMultQDA} proves to be the superior estimator ($pvalue = 0.0018$ vs. \texttt{QDAmod}), suggesting that constraining the two classes to share a common permutation symmetry provides a more effective regularization than allowing for fully independent covariance structures or using the standard \texttt{gipsQDA} approach ($pvalue = 0.0386$).
...
\textbf{Analysis:}
This dataset highlights the exceptional data efficiency of the \texttt{gipsLDA classic} model, which achieves robust performance even with minimal training samples. The statistical comparison confirms that imposing symmetry yields a significant advantage over the regularized baseline, with \texttt{gipsLDA classic} outperforming \texttt{LDAmod} ($pvalue = 0.0208$). Among the quadratic models, the benefit of the \texttt{gips} methodology is even more pronounced; \texttt{gipsMultQDA} achieves a highly significant improvement over \texttt{QDAmod} ($pvalue < 0.001$). This aligns with the domain intuition that geometric features of cell nuclei (e.g., radius, texture) share strong, structured correlations that are effectively captured by permutation symmetry. Conversely, the \texttt{gipsLDA weighted average} variant fails to improve upon the baseline ($pvalue = 0.8234$), visibly plateauing at a lower accuracy as the sample size increases.
...
\textbf{Analysis:}
The results for this dataset reveal a striking dichotomy between linear and quadratic classifiers. The data appears to possess an inherently linear decision boundary, as evidenced by the \texttt{LDA} family achieving accuracies above $0.80$, while all \texttt{QDA} variants fail to surpass $0.30$, likely due to the parameter explosion associated with estimating 11 distinct $48 \times 48$ covariance matrices. In this specific context, the \texttt{gips}-based regularization does not provide a statistically significant advantage over the baselines (all p-values $> 0.9$). The high p-values indicate that the standard regularized \texttt{LDAmod} is sufficient and potentially optimal; imposing additional permutation symmetries does not refine the decision boundary further, though notably, the \texttt{gipsLDA} models maintain competitive performance without degrading the results.
...
\textbf{Analysis:}
In this fraud detection scenario, characterized by a balanced undersampled dataset, the \texttt{gipsLDA classic} estimator demonstrates a distinct advantage. The statistical analysis confirms a significant improvement over the \texttt{LDAmod} baseline ($pvalue = 0.0062$), suggesting that the pooled covariance matrix benefits effectively from the imposed permutation constraint. Conversely, within the quadratic family, the \texttt{gips}-based regularization does not yield a statistically significant gain over standard \texttt{QDAmod} ($pvalue=1$), as all quadratic variants converge to a similarly high accuracy plateau. Notably, the plot highlights the suboptimal performance of the \texttt{gipsLDA weighted average} estimator, which fails to converge to the level of the other classifiers, reinforcing the observation that the Bayesian Model Averaging approach can be detrimental in specific homoscedastic settings.
...
\textbf{Analysis:}
The results for the EEG Brainwave dataset reveal a distinct separation between linear and quadratic classifiers, with the linear family consistently achieving higher accuracy. This suggests that the covariance structures of the different emotional states are largely similar (homoscedastic). Within this favorable linear regime, the \texttt{gips} methodology provides a tangible benefit: \texttt{gipsLDA classic} achieves a statistically significant improvement over the \texttt{LDAmod} baseline ($pvalue = 0.038$). Conversely, the quadratic models start with lower accuracy and converge more slowly, likely due to the increased parameter overhead relative to the sample size. Consequently, no significant advantage was found for the \texttt{gips}-based quadratic variants over standard \texttt{QDAmod} in this specific context ($pvalue > 0.05$).
...
\textbf{Analysis:}
In this multi-class industrial classification task, the data exhibits a structure that strongly favors linear decision boundaries. The linear models (\texttt{LDAmod} and \texttt{gipsLDA classic}) consistently outperform their quadratic counterparts, achieving an asymptotic accuracy of approximately 70\%. The statistical analysis yields a p-value of $0.2888$ for the comparison between \texttt{gipsLDA classic} and \texttt{LDAmod}, indicating that while the permutation constraint maintains competitive performance, it does not offer a statistically significant improvement over the standard regularized baseline in this specific context. Conversely, the quadratic models and the \texttt{gipsLDA weighted average} estimator struggle significantly in the low-sample regime, likely due to the increased estimation variance inherent in the multi-class setting (7 classes), which exacerbates the curse of dimensionality.


\section{Summary of Findings}

Based on the detailed analysis of both the synthetic simulation sets and the diverse real-world benchmarks, we can synthesize the following key findings regarding the proposed \texttt{gipsDA} framework:

\begin{itemize}
    \item \textbf{Efficacy in High-Dimensional, Data-Scarce Regimes:}
    The synthetic experiments conclusively demonstrate that imposing permutation symmetry acts as a powerful form of model-based regularization. In scenarios characterized by high dimensionality ($p=10$) and low sample sizes ($n \approx p$ or $n < p$), the \texttt{gips}-based models significantly outperformed the standard regularized baselines. Specifically, \texttt{gipsMultQDA} and \texttt{gipsQDA} consistently achieved p-values near zero when compared to \texttt{QDAmod}, proving that reducing the parameter space via symmetry constraints effectively mitigates the "curse of dimensionality" and prevents overfitting.

    \item \textbf{Superiority of the Classic Estimator for gipsLDA:}
    A recurrent pattern across both synthetic and real-world results (e.g., Credit Card, Steel Plates) is the robust performance of the \texttt{gipsLDA classic} estimator. This approach, which pools the covariance matrices before projecting onto the symmetry group, consistently outperformed the \texttt{gipsLDA weighted average} variant. The latter, which utilizes Bayesian Model Averaging, often failed to converge to optimal accuracy in higher-dimensional or multi-class settings. Consequently, the \texttt{classic} estimator is recommended as the default choice for homoscedastic modeling.

    \item \textbf{Data Efficiency in Real-World Applications:}
    The benchmarking on real-world datasets highlighted the "data efficiency" of the proposed models. In the Breast Cancer and Heart Failure datasets, \texttt{gipsLDA classic} and \texttt{gipsMultQDA} reached their asymptotic accuracy levels with significantly fewer training samples than their unstructured counterparts. This confirms that discovering and enforcing latent symmetries allows the models to learn the decision boundary faster, a critical advantage in domains where data acquisition is costly.
    ...
    Finally, the real-world benchmarks (specifically Steel Plates, Sensorless, and EEG) underscored a practical reality: many industrial datasets exhibit inherently linear separability. In these cases, while quadratic models (both standard and \texttt{gips}-based) struggled due to parameter complexity, the linear \texttt{gipsLDA} models maintained high performance, proving to be a safe and stable choice that does not degrade results even when the symmetry assumption is an approximation.
\end{itemize}
...
This thesis set out to address the limitations of classical discriminant analysis in high-dimensional, data-scarce environments. By developing the \texttt{gipsDA} package, we successfully integrated the theory of permutation-invariant covariance estimation with standard classification workflows. This final chapter summarizes the broader implications of our approach, reflects on the engineering challenges encountered, and outlines potential avenues for future research.

\section{Reflection on the Approach}
\label{sec:reflection}

The core hypothesis of this work was that imposing permutation symmetry on the covariance matrix acts as a physically motivated form of regularization, superior to arbitrary shrinkage in settings where $p \gg n$. The empirical results presented in Chapter \ref{ch:results} largely validate this hypothesis.
...
\begin{itemize}
    ...
    \item \textbf{The "Classic" vs. "Weighted" Dilemma:} One of the most significant findings was the underperformance of the Bayesian Model Averaging (weighted average) strategy in the homoscedastic \texttt{gipsLDA} model. While theoretically robust, in practice, the ensemble of permutations often introduced unnecessary noise compared to the single, optimal structure found by the MAP estimator. This highlights a crucial insight: in discriminant analysis, a crisp, well-defined decision boundary (derived from a single permutation) is often preferable to a "smeared" boundary derived from a posterior average.
    \item \textbf{Practical Utility:} The seamless integration with the \texttt{MASS} library syntax proved to be a vital design decision. By lowering the barrier to entry, we ensured that the complex mathematical backend remains accessible to data scientists, bridging the gap between advanced statistical theory and practical application.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{CRAN Compliance and Publishing Standards:} We mastered the rigorous process of preparing an R package for the Comprehensive R Archive Network (CRAN). This involved ensuring cross-platform compatibility, strictly resolving all \texttt{R CMD check} notes and warnings, and sanitizing the code to meet the community's high-quality standards for public distribution.
...
\begin{table}[H]
    ...
    \textless{}None\textgreater{} & \textless{}Helping with Unit Tests for package\textgreater{} \\ \hline
    \end{tabularx}
